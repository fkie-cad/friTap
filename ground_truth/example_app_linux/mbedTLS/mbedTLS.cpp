...
{
    char* output = (char*)malloc(strlen((const char*)ssl.session->id) * 2);
    int loop;
    int i;
    ...
    for (int i = 0; i < ssl.session->id_len; i++)
    {
...
int main(void)
{
    ...
    if ((flags = mbedtls_ssl_get_verify_result(&ssl)) != 0)
    {
        ...
        fflush(stdout);
        mbedtls_printf("Size: %d", sizeof(size_t));
        fflush(stdout);
