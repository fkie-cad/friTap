...
export class OpenSSL_BoringSSL {
    ...
    constructor(public moduleName:string, public socket_library:String,is_base_hook: boolean ,public passed_library_method_mapping?: { [key: string]: Array<string> } ){
        ...
        resolveOffsets(this.addresses, this.moduleName, socket_library, "openssl");
                    ...
                    if (!ObjC.available){
                        ...
                        if (!this.is_base_hook) {
                            ...
                            if (error instanceof Error) {
                                devlog_error("Error: " + error.message);
                                devlog_error("Stack: " + error.stack);
                            } else {
                                devlog_error("Unexpected error: " + error);
                            }
                        }
        ...
        if (this.is_openssl){
            ...
            Interceptor.attach(this.addresses[this.moduleName]["SSL_read_ex"],
            {
                ...
                onEnter: function (args: any) 
                {
                    ...
                    this.buf = args[1]
                    this.readbytes_ptr = args[3]

                },
                onLeave: function (retval: any) {
                    retval |= 0 // Cast retval to 32-bit integer.
                    var actual_bytes = this.readbytes_ptr.readULong()
                    if (actual_bytes <= 0 || (this.fd < 0 && !enable_default_fd)) {
                        return
                    }
