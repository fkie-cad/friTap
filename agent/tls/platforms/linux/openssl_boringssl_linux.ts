...
import {OpenSSL_BoringSSL } from "../../libs/openssl_boringssl.js";
...
export class OpenSSL_BoringSSL_Linux extends OpenSSL_BoringSSL {
    ...
    install_openssl_key_extraction_hook(): PatternBasedHooking | null {
        // If pipeline already resolved ssl_log_secret, hook directly (no PatternBasedHooking needed)
        const sslLogSecretAddr = this.addresses[this.module_name]?.["ssl_log_secret"];
        if (sslLogSecretAddr && !sslLogSecretAddr.isNull()) {
            devlog("[Pipeline] ssl_log_secret resolved by pipeline, hooking directly");
            Interceptor.attach(sslLogSecretAddr, {
                onEnter: (args) => {
        ...
        // Fallback: existing PatternBasedHooking approach (hardcoded patterns)
...
export function boring_execute(moduleName:string, is_base_hook: boolean){
    executeSSLLibrary(OpenSSL_BoringSSL_Linux, moduleName, socket_library, is_base_hook);
}

export function ssl_python_execute(moduleName:string, is_base_hook: boolean){
    executeSSLLibrary(OpenSSL_From_Python_Linux, moduleName, socket_library, is_base_hook);
}